<a href="{{ $post->getUrl() }}" class="block w-full h-auto lg:h-72 lg:flex mx-auto bg-tertiary no-underline mb-8">
    @if ($post->img)
        <div class="flex-1 w-full lg:w-80 h-48 lg:h-auto flex-none bg-center bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden" style="background-image: url('{{ $post->imgpath }}{{ $post->img }}');"></div>
    @else
        <div class="">BLOG</div>
    @endif
