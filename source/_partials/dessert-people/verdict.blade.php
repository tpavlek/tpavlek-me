<div class="bg-grey-100 rounded @if (!isset($preserve_margin) || !$preserve_margin) md:-mx-8 @endif @if (!isset($disable_shadow) || !$disable_shadow) shadow-md @endif relative overflow-hidden flex flex-wrap md:divide-x-2 md:divide-grey-400">
    <div class="flex w-full md:w-1/2 min-h-40">
        <div class="w-1/3 flex-none bg-cover bg-top overflow-hidden relative" style="background-image:url('/img/dessert-people/verdict/troy_{{ str_replace('.', '_', $troy_stars) }}_stars.jpg');">
        ...
        <div class="p-4 text-left text-grey-900">
            ...
            <p class="mt-2 leading-loose text-base">
                {!! (isset($troy_verdict)) ? (new Parsedown())->parse($troy_verdict) : '' !!}
            </p>
        </div>
    </div>
    <div class="flex w-full md:w-1/2 min-h-40">
        <div class="p-4 text-left text-grey-900 flex-grow">
            ...
            <p class="mt-2 leading-loose text-base">
                {!! (isset($rhi_verdict)) ? (new Parsedown())->parse($rhi_verdict) : '' !!}
            </p>
        </div>
        <div class="w-1/3 flex-none bg-cover bg-top overflow-hidden relative" style="background-image:url('/img/dessert-people/verdict/rhi_{{ str_replace('.', '_', $rhi_stars) }}_stars.jpg');">
